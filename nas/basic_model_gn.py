...
from basic_operation_gn import *
...
class Network(nn.Module):
    ...
    def __init__(self, genotype_list, layers=9, C=36, num_classes=5, steps=2, multiplier=2, stem_multiplier=3,
                 AdPoolSize=7):
        ...
        C_curr = stem_multiplier * C  # C_curr 108
        self.stem = nn.Sequential(
            nn.Conv2d(3, C_curr, 3, padding=1, bias=False),
            nn.GroupNorm(6, C_curr)
        )

        C_prev_prev, C_prev, C_curr = C_curr, C_curr, C  # 108 108 36
        ...
        self.classifier = nn.Sequential(
            nn.Linear(C_prev*AdPoolSize*AdPoolSize, 4096),
            nn.ReLU(True),
            nn.Dropout(),
            nn.Linear(4096, 4096),
            nn.ReLU(True),
